...
def main_swiss_roll_partial():
    ...
    max_iter_finsler = 12              # 150
    inner_iter_finsler = 4             # 5
    max_iter_wormhole = 20            # 1000
    ...
    utils.set_window_title(fig, "Swiss roll partial: full input")
    ...
    fig_isomap, ax_isomap = utils.plot_proj_points(proj, X=X, knn=knn, X_noiseless=X_noiseless, shape_type='swiss_roll',
                                                   edge_alpha=1.)
    utils.set_window_title(fig_isomap, "Swiss roll partial: full Isomap")
    ax_isomap.axis('off')
    ...
    utils.set_window_title(fig_randers, f"Swiss roll partial: full Randers field alpha={randers_w_alpha_manifold}")

    #fig_randers.savefig(os.path.join(dir_res, 'swiss_roll_partial_randers_full.pdf'))


    print('Finsler path-frozen on full')
    ...
    if not os.path.exists(os.path.join(dir_res_raw, 'proj_path_frozen_matsumoto_unif_' + param_str + '.npy')):
        # proj_smacof_f_unif, _ = fit_finsler_mds(
        #     dists_f, metric=RandersMetric(alpha=randers_w_alpha_embedding), optimizer="smacof_randers",
        #     init=init, n_components=proj_dim, n_init=1, n_jobs=1,
        #     weight=unif_weights * 1., max_iter=max_iter_finsler,
        #     pseudo_inv_solver="gmres", project_on_V=True,
        #     check_monotony=False,
        # )
        proj_smacof_f_unif, _ = fit_finsler_mds(
            dists_f,
            metric=MatsumotoMetric(alpha=randers_w_alpha_embedding),
            optimizer="path_frozen",
            init=init,
            n_components=proj_dim,
            weight=unif_weights * 1.,
            graph_neighbors=k,
            outer_iter=max_iter_finsler,
        ...
        proj_smacof_f_unif = np.load(os.path.join(dir_res_raw, 'proj_path_frozen_matsumoto_unif_' + param_str + '.npy'))
    ...
    utils.set_window_title(fig_smacof_f_unif, f"Swiss roll partial: full Matsumoto path-frozen alpha={randers_w_alpha_embedding}")
    ...
    utils.set_window_title(fig_hole, "Swiss roll hole: input")
    ...
    utils.set_window_title(fig_hole_randers, f"Swiss roll hole: Randers field alpha={randers_w_alpha_manifold}")
    ...
    fig_isomap_hole, ax_isomap_hole = utils.plot_proj_points(proj_hole, X=X_hole, knn=knn_hole,
                                                             ...
                                                             edge_alpha=1.)
    utils.set_window_title(fig_isomap_hole, "Swiss roll hole: Isomap")
    ...
    fig_smacof_unif_hole, ax_smacof_unif_hole = utils.plot_proj_points(proj_smacof_unif_hole, X=X_hole, knn=knn_hole,
                                                                        X_noiseless=X_noiseless_hole, shape_type='swiss_roll',
                                                                        edge_alpha=1.)
    utils.set_window_title(fig_smacof_unif_hole, "Swiss roll hole: vanilla wormhole SMACOF")
    ...
    utils.set_window_title(fig_dists_hole, f"Swiss roll hole: Finsler distances alpha={randers_w_alpha_manifold}")
    ...
    print('Vanilla Finsler path-frozen embedding')
    np.random.seed(seed + 4)
    unif_weights = np.ones_like(dists_f_hole)
    if not os.path.exists(os.path.join(dir_res_raw, 'proj_path_frozen_matsumoto_unif_hole_' + param_str + '.npy')):
        # proj_smacof_f_unif_hole, _ = fit_finsler_mds(
        #     dists_f_hole, metric=RandersMetric(alpha=randers_w_alpha_embedding), optimizer="smacof_randers",
        #     init=init, n_components=proj_dim, n_init=1, n_jobs=1,
        #     weight=unif_weights * 1., max_iter=max_iter_finsler,
        #     pseudo_inv_solver="gmres", project_on_V=True,
        #     check_monotony=False,
        ...
        proj_smacof_f_unif_hole = np.load(os.path.join(dir_res_raw, 'proj_path_frozen_matsumoto_unif_hole_' + param_str + '.npy'))

    fig_f_unif_hole, ax_f_unif_hole = utils.plot_proj_points(proj_smacof_f_unif_hole, X=X_hole, knn=knn_hole,
                                                             X_noiseless=X_noiseless_hole, shape_type='swiss_roll',
                                                             edge_alpha=1.)
    utils.set_window_title(fig_f_unif_hole, f"Swiss roll hole: Matsumoto path-frozen uniform alpha={randers_w_alpha_embedding}")
    # matplotlib.rcParams['text.usetex'] = True
    ...
    print('Wormhole Finsler path-frozen embedding')
    # For Randers SMACOF, we needed symmetric weights:
    ...
    if not os.path.exists(os.path.join(dir_res_raw, 'proj_path_frozen_matsumoto_wormhole_hole_' + param_str + '.npy')):
        # proj_smacof_f_wormhole_hole, _ = fit_finsler_mds(
        #     dists_f_hole, metric=RandersMetric(alpha=randers_w_alpha_embedding), optimizer="smacof_randers",
        #     init=init, n_components=proj_dim, n_init=1, n_jobs=1,
        #     weight=mask_wormhole_randers_hole_sym * 1., max_iter=max_iter_finsler,
        #     eps=1e-3,
        #     pseudo_inv_solver="gmres", project_on_V=True,
        #     check_monotony=False,
        # )
        proj_smacof_f_wormhole_hole, _ = fit_finsler_mds(
            ...
            init=init,
            n_components=proj_dim,
            weight=mask_wormhole_randers_hole * 1.,
            graph_neighbors=k,
            outer_iter=max_iter_finsler,
            inner_iter=inner_iter_finsler,
            eps=1e-3,
        ...
        proj_smacof_f_wormhole_hole = np.load(os.path.join(dir_res_raw, 'proj_path_frozen_matsumoto_wormhole_hole_' + param_str + '.npy'))
    ...
    utils.set_window_title(fig_f_wormhole_hole, f"Swiss roll hole: Matsumoto path-frozen wormhole alpha={randers_w_alpha_embedding}")
    # matplotlib.rcParams['text.usetex'] = True
